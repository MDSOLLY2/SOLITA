...
class S_O_L_L_Y(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
      ...
      self.highrise.tg.create_task(self.highrise.teleport(
          session_metadata.user_id, Position(x=12.5, y=7.5, z=2.5, facing='FrontRight')))
    ...
    async def on_user_join(self, user: User, position: Position) -> None:
      #يحيك
      await self.highrise.chat(f"Добро пожаловать во дворец 🔥🖤 Чтобы танцевать, используйте цифры от 1 до 68.")
      await self.highrise.chat(f"Вы можете остаться во дворце, чтобы получить золото и поучаствовать в главном призе.")
      await self.highrise.chat(f"Чтобы получить крупный приз, вам нужно выполнить всего два условия: первое — подписаться на мою страницу, а второе — комментировать все публикации на моей странице. Победитель будет объявлен, когда на моей странице будет 1500 подписчиков.")
    ...
    async def on_reaction(self, user: User, reaction: Reaction, receiver: User) -> None:
      ...
      if reaction =="heart"and user.username in ["S_O_L_L_Y"]:
      ...
      if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
              if reaction == "thumbs":
                await self.highrise.teleport(receiver.id, Position(x=7.5, y=4.099999904632568, z=2.5, facing='FrontLeft'))
              if reaction == "wave":
                await self.highrise.teleport(receiver.id, Position(x=11.5, y=0.6000000238418579, z=8.5, facing='FrontRight'))

      room_users = (await self.highrise.get_room_users()).content
      if user in [target_user for target_user, _ in room_users] and user.username not in ["SOLLY_PARTY"]:
          try:
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
      if tip.amount > 99 and receiver.username in ["S_O_L_L_Y","SOLLY_PARTY"]:
          await self.highrise.teleport(sender.id, Position(x=12.5, y=7.5, z=2.5, facing='FrontRight'))
    ...
    # ... (باقي الأوامر هنا)
      ...
      if message.startswith("back") and user.username in ["S_O_L_L_Y"]:
        await self.highrise.walk_to(Position(x=12.5, y=7.5, z=2.5, facing='FrontRight'))

      if message in ["Host","host","!Host","!host","هوست"]:
        user_privileges = await self.highrise.get_room_privilege(user.id)
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}", Position(x=12.5, y=7.5, z=2.5, facing='FrontRight'))
          except:
            print("error 3")
      ...
      if message in ["Down","down","!Down","!down","نزلني","نزلنى"]:
        try:
            await self.highrise.teleport(f"{user.id}", Position(x=11.5, y=0.6000000238418579, z=8.5, facing='FrontRight'))
        except:
          print("error 3")
      ...
      if message.lstrip().startswith("!tele"):
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          ...
          destinations = {
              'vip': Position(x=12.5, y=7.5, z=2.5, facing='FrontRight'),
              'down': Position(x=11.5, y=0.6000000238418579, z=8.5, facing='FrontRight'),
              'start' : Position(x=13.5, y=7.75, z=2.0, facing='FrontRight'),
          }
