...
class S_O_L_L_Y(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
      print("SOLLY_6CORNERS")
      self.highrise.tg.create_task(self.highrise.teleport(
          session_metadata.user_id, Position(x=8.5, y=7.25, z=0.5, facing='FrontRight')))
    ...
    async def on_reaction(self, user: User, reaction: Reaction, receiver: User) -> None:
      ...
      if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
            if reaction == "clap":
              await self.highrise.teleport(receiver.id, Position(x=10.5, y=0.0, z=0.5, facing='FrontRight'))
            if reaction == "thumbs":
              await self.highrise.teleport(receiver.id, Position(x=10.5, y=0.0, z=5.5, facing='FrontRight'))
            if reaction == "wave":
              await self.highrise.teleport(receiver.id, Position(x=8.5, y=7.25, z=1.5, facing='FrontRight'))
            if reaction == "wink":
              await self.highrise.teleport(receiver.id, Position(x=13.5, y=7.25, z=14.5, facing='FrontLeft'))

      room_users = (await self.highrise.get_room_users()).content
      if user in [target_user for target_user, _ in room_users] and user.username not in ["SOLLY_6CORNERS"]:
          try:
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
      if tip.amount > 4 and receiver.username in ["S_O_L_L_Y","SOLLY_6CORNERS"]:
          await self.highrise.teleport(sender.id, Position(x=10.5, y=0.0, z=5.5, facing='FrontRight'))
    ...
    # ... (باقي الأوامر هنا)
      ...
      if message.startswith("back") and user.username in ["S_O_L_L_Y"]:
        await self.highrise.walk_to(Position(x=10.5, y=7.25, z=0.5, facing='FrontRight'))

      if message in ["Host","host","!Host","!host","هوست"]:
        user_privileges = await self.highrise.get_room_privilege(user.id)
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}", Position(x=13.5, y=7.25, z=14.5, facing='FrontLeft'))
          except:
            print("error 3")
      ...
      if message in ["in","play","In","Play","لعبني"]:
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}",Position(x=10.5, y=0.0, z=5.5, facing='FrontRight'))
          except Exception as e:
            print(f"Error: {e}")
      ...
      if message.lstrip().startswith("!tele"):
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          ...
          destinations = {
            'host': Position(x=13.5, y=7.25, z=14.5, facing='FrontLeft'),
            'in': Position(x=10.5, y=0.0, z=5.5, facing='FrontRight'),
            'door': Position(x=8.5, y=7.25, z=1.5, facing='FrontRight'),
            'start' : Position(x=10.5, y=7.25, z=0.5, facing='FrontRight'),
          }
