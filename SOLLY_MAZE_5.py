...
class S_O_L_L_Y(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
      ...
      self.highrise.tg.create_task(self.highrise.teleport(
          session_metadata.user_id, Position(x=17.5, y=2.25, z=26.5, facing='FrontLeft')))
    ...
    async def on_user_join(self, user: User, position: Position) -> None:
      ...
      #يرجعك للباب
      await self.highrise.teleport(user.id, Position(x=17.5, y=2.0, z=28.5, facing='FrontLeft'))

      #يحيك
      await self.highrise.chat(f"Welcome to Solly's maze room, U can tip 10G to the bot to play the maze. The winner will get 50G, Hope y'all enjoy and win, GL {user.username} 💖")
    ...
    async def on_whisper(self, user: User, message: str) -> None:
          ...
          if message.startswith("in all"):
              ...
              for roomUser, _ in roomUsers:
                await self.highrise.teleport(f"{roomUser.id}", Position(x=12.5, y=0.25, z=25.5, facing='FrontLeft'))

          if message.startswith("out all"):
              roomUsers = (await self.highrise.get_room_users()).content
              for roomUser, _ in roomUsers:
                await self.highrise.teleport(f"{roomUser.id}", Position(x=17.5, y=2.0, z=28.5, facing='FrontLeft'))


    #لو عايز الهوست ينقل حد بيراكت
    async def on_reaction(self, user: User, reaction: Reaction, receiver: User) -> None:

      if reaction =="heart"and user.username in ["S_O_L_L_Y","SOLLY_MAZE_5"]:
      ...
      if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
                if reaction == "clap":
                  await self.highrise.teleport(receiver.id, Position(x=17.5, y=0.5, z=4.5, facing='FrontRight'))
                if reaction == "thumbs":
                  await self.highrise.teleport(receiver.id, Position(x=12.5, y=0.25, z=25.5, facing='FrontLeft'))
                if reaction == "wave":
                  await self.highrise.teleport(receiver.id, Position(x=17.5, y=2.0, z=28.5, facing='FrontLeft'))
                if reaction == "wink":
                  await self.highrise.teleport(receiver.id, Position(x=9.5, y=0.5, z=0.5, facing='FrontRight'))

      room_users = (await self.highrise.get_room_users()).content
      if user in [target_user for target_user, _ in room_users] and user.username not in ["SOLLY_MAZE_5"]:
          try:
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
      if tip.amount > 9 and receiver.username in ["S_O_L_L_Y","SOLLY_MAZE_5"]:
          await self.highrise.teleport(sender.id, Position(x=12.5, y=0.25, z=25.5, facing='FrontLeft'))
    ...
    # ... (باقي الأوامر هنا)
      ...
      if message.startswith("back") and user.username in ["S_O_L_L_Y"]:
        await self.highrise.walk_to(Position(x=17.5, y=3.25, z=23.5, facing='FrontLeft'))

      if message in ["Host","host","!Host","!host","هوست"]:
        user_privileges = await self.highrise.get_room_privilege(user.id)
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}", Position(x=9.5, y=0.5, z=0.5, facing='FrontRight'))
          except:
            print("error 3")
      ...
      if message in ["in","play","In","Play","لعبني"]:
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}",Position(x=12.5, y=0.25, z=25.5, facing='FrontLeft'))
          except Exception as e:
            print(f"Error: {e}")
      ...
      if message in ["خرجني","door","Door","Out","out"]:
        try:
            await self.highrise.teleport(f"{user.id}", Position(x=17.5, y=2.0, z=28.5, facing='FrontLeft'))
        except:
          print("error 3")
      ...
      if message.lstrip().startswith("!tele"):
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          ...
          destinations = {
            'host': Position(x=9.5, y=0.5, z=0.5, facing='FrontRight'),
            'in':Position(x=12.5, y=0.25, z=25.5, facing='FrontLeft'),
            'door': Position(x=17.5, y=2.0, z=28.5, facing='FrontLeft'),
            'start' : Position(x=17.5, y=3.25, z=23.5, facing='FrontLeft'),
          }
