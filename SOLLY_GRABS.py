...
class S_O_L_L_Y(BaseBot):
    ...
    async def on_whisper(self, user: User, message: str) -> None:
          ...
          if message.startswith("in all"):
              ...
              for roomUser, _ in roomUsers:
                  if roomUser.username not in ["SOLLY_GRABS"]:
                    await self.highrise.teleport(f"{roomUser.id}", Position(x=8.5, y=0.5, z=0.5, facing='FrontRight'))

          if message.startswith("out all"):
              roomUsers = (await self.highrise.get_room_users()).content
              for roomUser, _ in roomUsers:
                  if roomUser.username not in ["SOLLY_GRABS"]:
                    await self.highrise.teleport(f"{roomUser.id}", Position(x=19.5, y=0.0, z=12.5, facing='FrontLeft'))
    ...
    async def on_reaction(self, user: User, reaction: Reaction, receiver: User) -> None:
      ...
      if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
              if reaction == "thumbs":
                await self.highrise.teleport(receiver.id, Position(x=8.5, y=0.5, z=0.5, facing='FrontRight'))
              if reaction == "wave":
    ...
    # ... (باقي الأوامر هنا)
      ...
      if message in ["Host","host","!Host","!host","هوست"]:
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}", Position(x=7.5, y=0.0, z=17.5, facing='FrontLeft'))
          except:
            print("error 3")

      if message in ["Hoost","hoost","!Hoost","!hoost","هووست"]:
        user_privileges = await self.highrise.get_room_privilege(user.id)
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}", Position(x=18.0, y=0.0, z=10.0, facing='FrontLeft'))
          except:
            print("error 3")
      ...
      if message in ["in","play","In","Play","لعبني","نزلني"]:
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}",Position(x=8.5, y=0.5, z=0.5, facing='FrontRight'))
          except Exception as e:
            print(f"Error: {e}")
      ...
      if message in ["خرجني","door","Door","Out","out","طلعني","طلعنى","اوت"]:
        try:
            await self.highrise.teleport(f"{user.id}", Position(x=19.5, y=0.0, z=12.5, facing='FrontLeft'))
        except:
          print("error 3")
      ...
      if message.lstrip().startswith("!tele"):
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          ...
          destinations = {
            'host': Position(x=7.5, y=0.0, z=17.5, facing='FrontLeft'),
            'in': Position(x=8.5, y=0.5, z=0.5, facing='FrontRight'),
            'door': Position(x=19.5, y=0.0, z=12.5, facing='FrontLeft'),
            'hoost' : Position(x=18.0, y=0.0, z=10.0, facing='FrontLeft'),
            'start' : Position(x=19.5, y=0.0, z=12.5, facing='FrontLeft'),
          }
