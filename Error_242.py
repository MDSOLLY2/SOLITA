...
class S_O_L_L_Y(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
      try:
          print("Error_242")
          await self.highrise.walk_to(Position(x=16.5, y=0.0, z=22.5, facing='FrontRight'))
    ...
    async def handle_order(self, user: User, message: str):
      ...
      if total_price > 0:
          ...
          await self.highrise.walk_to(Position(x=6.5, y=0.0, z=16.0, facing='FrontRight'))
    ...
    #يحيك
      if user.username in ["5RI___","P21"]:
      ...
      if user.username not in ["_xA7m3d"]:
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
      if tip.amount == 10 and receiver.username in ["Error_242"]:
        await asyncio.sleep(1)
      try:
          emote_id = random.choice(self.dances)
      ...
      if tip.amount == 5 and receiver.username in ["Error_242"]:
        await asyncio.sleep(1)
        await self.highrise.teleport(sender.id, Position(x=17.5, y=14.350000381469727, z=24.5, facing='FrontRight'))
    ...
    # ... (باقي الأوامر هنا)
      ...
      if message.startswith(("React", "react", "وزع")) and user.username in ["S_O_L_L_Y","5RI___"]:
      ...
      if message.startswith(("محفظتك","wallet")):
        user_privileges = await self.highrise.get_room_privilege(user.id)
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y","5RI___"]):
      ...
      if message.startswith(("vip","Vip")):
        user_privileges = await self.highrise.get_room_privilege(user.id)
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y","5RI___"]):
          await self.highrise.teleport(user.id, Position(x=17.5, y=14.350000381469727, z=24.5, facing='FrontRight'))
      ...
      if message in ["نزلني","دور 1","نزلنى"]:
        try:
            await self.highrise.teleport(f"{user.id}", Position(x=15.5, y=0.10000000149011612, z=29.5, facing='FrontRight'))
        except:
          print("error 3")

      if  message in ["طلعني","صعدني","صعدنى","طلعنى"]:
        try:
            await self.highrise.teleport(f"{user.id}", Position(x=12.5, y=7.25, z=25.5, facing='FrontRight'))
        except:
          print("error 3")

      if message in ["طيرنى","طيرني"]:
        await self.highrise.teleport(f"{user.id}", Position(x=17.5, y=14.350000381469727, z=24.5, facing='FrontRight'))
        await asyncio.sleep (3)
        await self.highrise.teleport(f"{user.id}", Position(x=12.5, y=7.25, z=25.5, facing='FrontRight'))
        await asyncio.sleep (3)
        await self.highrise.teleport(f"{user.id}", Position(x=15.5, y=0.10000000149011612, z=29.5, facing='FrontRight'))
        await asyncio.sleep(3)
        await self.highrise.teleport(f"{user.id}", Position(x=16.5, y=0.0, z=22.5, facing='FrontRight'))
        await asyncio.sleep(3)
        await self.highrise.teleport(f"{user.id}", Position(x=3.0, y=0.0, z=3.0, facing='FrontRight'))
        await asyncio.sleep(3)
        await self.highrise.teleport(f"{user.id}", Position(x=6.5, y=0.0, z=16.0, facing='FrontRight'))
      ...
      if message.lstrip().startswith(("add","!tele","انقل")):
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y","5RI___"]):
          ...
          destinations = {
            'vip2': Position(x=17.5, y=14.350000381469727, z=24.5, facing='FrontRight'),
            'vip': Position(x=12.5, y=7.25, z=25.5, facing='FrontRight'),
            'down' : Position(x=15.5, y=0.10000000149011612, z=29.5, facing='FrontRight'),
          }
      ...
      if message.startswith("تبرع"):
        try:
            ...
            return
        if user.username in [ "sll0", "_xA7m3d" ]:
            response = await self.highrise.get_room_users()
