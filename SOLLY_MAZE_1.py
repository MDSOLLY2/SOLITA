...
class S_O_L_L_Y(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
      ...
      self.highrise.tg.create_task(self.highrise.teleport(
          session_metadata.user_id, Position(4, 0, 0.5, "FrontRight")))
    ...
    async def on_user_join(self, user: User, position: Position) -> None:
      ...
      #يرجعك للباب
      await self.highrise.teleport(user.id, Position(1.5, 0.0, 1.5, "FrontRight"))

      #يحيك
      await self.highrise.chat(f"Welcome to Solly's maze room, U can tip 10G to the bot to play the maze. The winner will get 500G, Hope y'all enjoy and win, GL {user.username} 💖")
    ...
    async def on_whisper(self, user: User, message: str) -> None:
          ...
          if message.startswith("in all"):
              ...
              for roomUser, _ in roomUsers:
                await self.highrise.teleport(f"{roomUser.id}", Position(17.5, 0.25, 29.5, "FrontLeft"))

          if message.startswith("out all"):
              roomUsers = (await self.highrise.get_room_users()).content
              for roomUser, _ in roomUsers:
                await self.highrise.teleport(f"{roomUser.id}", Position(1.5, 0.0, 1.5, "FrontRight"))


    #لو عايز الهوست ينقل حد بيراكت
    async def on_reaction(self, user: User, reaction: Reaction, receiver: User) -> None:

      if reaction =="heart"and user.username in ["S_O_L_L_Y","SOLLY_MAZE_1"]:
      ...
      if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
                ...
                if reaction == "thumbs":
                  await self.highrise.teleport(receiver.id, Position(17.5, 0.25, 29.5, "FrontLeft"))
                if reaction == "wave":
                  await self.highrise.teleport(receiver.id, Position(1.5, 0.0, 1.5, "FrontRight"))
                if reaction == "wink":
                  await self.highrise.teleport(receiver.id, Position(15.5, 0.25,0.5, "FrontRight"))

      room_users = (await self.highrise.get_room_users()).content
      if user in [target_user for target_user, _ in room_users] and user.username not in ["SOLLY_MAZE_1"]:
          try:
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
      if tip.amount > 9 and receiver.username in ["S_O_L_L_Y","SOLLY_MAZE_1"]:
          await self.highrise.teleport(sender.id, Position(17.5, 0.25, 29.5, "FrontLeft"))
    ...
    # ... (باقي الأوامر هنا)
      ...
      if message.startswith("back") and user.username in ["S_O_L_L_Y"]:
        await self.highrise.walk_to(Position(6.5, 0.25, 0.5, "FrontRight"))

      if message in ["Host","host","!Host","!host","هوست"]:
        user_privileges = await self.highrise.get_room_privilege(user.id)
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}", Position(15.5, 0.25,0.5, "FrontRight"))
          except:
            print("error 3")
      ...
      if message in ["in","play","In","Play","لعبني"]:
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}",Position(17.5, 0.25, 29.5, "FrontLeft"))
          except Exception as e:
            print(f"Error: {e}")
      ...
      if message in ["خرجني","door","Door","Out","out"]:
        try:
            await self.highrise.teleport(f"{user.id}", Position(1.5, 0.0, 1.5, "FrontRight"))
        except:
          print("error 3")
      ...
      if message.lstrip().startswith("!tele"):
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          ...
          destinations = {
            'host': Position(15.5, 0.25,0.5, "FrontRight"),
            'in': Position(17.5, 0.25, 29.5, "FrontLeft"),
            'door': Position(1.5, 0.0, 1.5, "FrontRight"),
            'start' : Position(6.5, 0.25, 0.5, "FrontRight"),
          }
