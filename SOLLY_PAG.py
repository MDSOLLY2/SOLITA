...
class S_O_L_L_Y(BaseBot):
    ...
    async def handle_order(self, user: User, message: str):
      ...
      if total_price > 0:
          ...
          await self.highrise.walk_to(Position(x=8.0, y=0.0, z=9.5, facing='FrontRight'))
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
      if tip.amount == 10 and receiver.username in ["S_O_L_L_Y"]:
      ...
      if tip.amount == 5 and receiver.username in ["S_O_L_L_Y"]:
        await asyncio.sleep(1)
        await self.highrise.teleport(sender.id, Position(15,0,15))
    ...
    # ... (باقي الأوامر هنا)
      ...
      if message in ["طيرنى","طيرني"]:
        ...
        await asyncio.sleep (3)
        await self.highrise.teleport(f"{user.id}", Position(x=11.5, y=7.25, z=15.0, facing='FrontLeft'))
        await asyncio.sleep (3)
        await self.highrise.teleport(f"{user.id}", Position(x=16.5, y=0.25, z=10.5, facing='FrontLeft'))
        await asyncio.sleep(3)
        await self.highrise.teleport(f"{user.id}", Position(x=3.5, y=7.25, z=6.5, facing='FrontRight'))
        await asyncio.sleep(3)
        await self.highrise.teleport(f"{user.id}", Position(x=4.0, y=0.0, z=7.5, facing='FrontRight'))
        await asyncio.sleep(3)
        await self.highrise.teleport(f"{user.id}", Position(x=5.5, y=0.25, z=25.5, facing='FrontRight'))
