...
class S_O_L_L_Y(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
      ...
      self.highrise.tg.create_task(self.highrise.teleport(
          session_metadata.user_id, Position(x=10.5, y=0.0, z=1.0, facing='FrontRight')))
    ...
    async def on_user_join(self, user: User, position: Position) -> None:
      ...
      #يرجعك للباب
      await self.highrise.teleport(user.id, Position(x=7.5, y=0.0, z=1.0, facing='FrontRight'))
    ...
    async def on_whisper(self, user: User, message: str) -> None:
          ...
          if message.startswith("in all"):
              ...
              for roomUser, _ in roomUsers:
                await self.highrise.teleport(f"{roomUser.id}", Position(x=17.5, y=0.25, z=25.5, facing='FrontLeft'))

          if message.startswith("out all"):
              roomUsers = (await self.highrise.get_room_users()).content
              for roomUser, _ in roomUsers:
                await self.highrise.teleport(f"{roomUser.id}", Position(x=7.5, y=0.0, z=1.0, facing='FrontRight'))


    #لو عايز الهوست ينقل حد بيراكت
    async def on_reaction(self, user: User, reaction: Reaction, receiver: User) -> None:

      if reaction =="heart"and user.username in ["S_O_L_L_Y","SOLLY_MAZE_2"]:
      ...
      if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
        if reaction == "clap":
          await self.highrise.teleport(receiver.id, Position(x=15.5, y=0.25, z=15.5, facing='FrontLeft'))
        if reaction == "thumbs":
          await self.highrise.teleport(receiver.id, Position(x=17.5, y=0.25, z=25.5, facing='FrontLeft'))
        if reaction == "wave":
          await self.highrise.teleport(receiver.id, Position(x=7.5, y=0.0, z=1.0, facing='FrontRight'))
        if reaction == "wink":
          await self.highrise.teleport(receiver.id, Position(x=14.5, y=0.25, z=1.5, facing='FrontRight'))

      room_users = (await self.highrise.get_room_users()).content
      if user in [target_user for target_user, _ in room_users] and user.username not in ["SOLLY_MAZE_2"]:
          try:
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
      if tip.amount > 9 and receiver.username in ["S_O_L_L_Y","SOLLY_MAZE_2"]:
          await self.highrise.teleport(sender.id, Position(x=17.5, y=0.25, z=25.5, facing='FrontLeft'))
    ...
    # ... (باقي الأوامر هنا)
      ...
      if message.startswith("back") and user.username in ["S_O_L_L_Y"]:
        await self.highrise.walk_to(Position(x=10.5, y=0.0, z=1.0, facing='FrontRight'))

      if message in ["Host","host","!Host","!host","هوست"]:
        user_privileges = await self.highrise.get_room_privilege(user.id)
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}", Position(x=14.5, y=0.25, z=1.5, facing='FrontRight'))
          except:
            print("error 3")
      ...
      if message in ["in","play","In","Play","لعبني"]:
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          try:
            await self.highrise.teleport(f"{user.id}",Position(x=17.5, y=0.25, z=25.5, facing='FrontLeft'))
          except Exception as e:
            print(f"Error: {e}")
      ...
      if message in ["خرجني","door","Door","Out","out"]:
        try:
            await self.highrise.teleport(f"{user.id}", Position(x=7.5, y=0.0, z=1.0, facing='FrontRight'))
        except:
          print("error 3")
      ...
      if message.lstrip().startswith("!tele"):
        ...
        if (user_privileges.moderator) or (user.username in ["S_O_L_L_Y"]):
          ...
          destinations = {
            'host': Position(x=14.5, y=0.25, z=1.5, facing='FrontRight'),
            'in': Position(x=17.5, y=0.25, z=25.5, facing='FrontLeft'),
            'door': Position(x=7.5, y=0.0, z=1.0, facing='FrontRight'),
            'start' : Position(x=10.5, y=0.0, z=1.0, facing='FrontRight'),
          }
